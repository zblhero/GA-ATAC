...
class Posterior:
    ...
    @torch.no_grad()
    def clustering_scores(self, latent, labels, cells, dataset, suffix, tlabels, louvain_num=15, prediction_algorithm="knn", X_tf=None, ensemble=False, batch_indices=None, save_cluster=False, seed=42):
        print('n labels', self.gene_dataset.n_labels, prediction_algorithm)
        for k in [1]:
            #if self.gene_dataset.n_labels > 1:
            if True:
                #latent, _, labels = self.sequential().get_latent()
                ...
                elif prediction_algorithm == "gmm-tsne":
                    gmm = GMM(self.gene_dataset.n_labels)
                    gmm.fit(tsne)
                    labels_pred = gmm.predict(latent)
                elif prediction_algorithm == "knn-tsne":
                    labels_pred = KMeans(
                        self.gene_dataset.n_labels, n_init=200
                    ).fit_predict(
                        tsne
                    )  # n_jobs>1 ?
                elif prediction_algorithm == 'sc':
                    from sklearn.cluster import SpectralClustering
                    mat = kneighbors_graph(latent, 15, mode='distance', include_self=True).todense()
                    labels_pred = SpectralClustering(n_clusters=12, affinity='precomputed').fit_predict(mat)
                    vec = latent
                elif prediction_algorithm == 'louvain':
                ...
                show_tsne(tsne, labels_pred, 'result/%s/%s-GMVAE-%s-%s-pred.png'%(dataset, suffix, self.model.reconstruction_loss, ensemble))
                np.savetxt('result/%s/labels.txt'%(dataset), labels_pred)
                
                if labels is not None:   
                    show_tsne(tsne, labels, 'result/%s/%s-GMVAE-%s-%s-true.png'%(dataset, suffix, self.model.reconstruction_loss, ensemble), tlabels=tlabels)
                    ...
                    logger.debug(
                        "Clustering Scores:\nSilhouette: %.4f\nNMI: %.4f\nARI: %.4f\nUCA: %.4f"
                        % (asw_score, nmi_score, ari_score, 0)
                    )
                    print(k, self.gene_dataset.n_labels, "Clustering Scores:\nSilhouette: %.4f\nNMI: %.4f\nARI: %.4f\nUCA: %.4f"
                        % (asw_score, nmi_score, ari_score, 0))
                if batch_indices is not None:
                    show_tsne(tsne, batch_indices, 'result/%s/%s-GMVAE-%s-%s-batch.png'%(dataset, suffix, self.model.reconstruction_loss, ensemble), tlabels=batch_indices)
    ...
    def show_t_sne(
        ...
        if latent is None:
            latent, batch_indices, labels = self.get_latent(sample=True)   # get latent and apply tsne
        ...
        if color_by == "scalar":
            ...
            if color_by == "batches" or color_by == "labels":
                ...
                plt.figure(figsize=(10, 10))
                colors = ['blue', 'orange', 'green', 'red', 'purple', 'brown', 'pink', 'yellow', 'black', 'teal', 'plum', 'tan', 'bisque', 'beige', 'slategray']
                for i, label in zip(range(n), plt_labels):
                    plt.scatter(
                        latent[indices == i, 0], latent[indices == i, 1], label=label, c=colors[i]
                    )
                plt.legend()
    ...
    def apply_t_sne(latent, n_samples=1000):
        ...
        #print('apply tsne', latent.shape, idx_t_sne)
        if latent.shape[1] != 2:
